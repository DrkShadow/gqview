...
static void view_collection_step(ViewWindow *vw, gint next) {
	...
	if (info) {
		...
		if (read_ahead_info)
			image_prebuffer_set(imd, read_ahead_info->path);
	}
...
static ViewWindow *real_view_window_new(const gchar *path, GList *list, CollectionData *cd, CollectInfo *info) {
	...
	if (black_window_background)
		image_background_set_black(vw->imd, TRUE);
	...
	if (cd && info) {
		...
		if (enable_read_ahead) {
			...
			if (r_info)
				image_prebuffer_set(vw->imd, r_info->path);
		}
	}
	else if (list) {
		...
		if (enable_read_ahead) {
			...
			if (work)
				image_prebuffer_set(vw->imd, (gchar *)work->data);
		}
	}
	else {
...
static void view_window_get_dnd_data(GtkWidget *widget, GdkDragContext *context,
	...
	if (gtk_drag_get_source_widget(context) == vw->imd->pr)
		return;
