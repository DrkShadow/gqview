...
gint comment_read(const gchar *path, GList **keywords, gchar **comment) {
	...
	while (fgets(s_buf,sizeof(s_buf), f)) {
		...
		if (s_buf[0]=='[') {
			...
			if (strcasecmp(key, "keywords") == 0) {
				if (strlen(s_buf) > 0)
					list = g_list_prepend(list, g_strdup(s_buf));
			}
			else if (strcasecmp(key, "comment") == 0) {
				if (!comment_build)
					comment_build = g_string_new("");
				g_string_append(comment_build, s_buf);
				if (strlen(s_buf) > 0 && newline)
					g_string_append_c(comment_build, '\n');
			}
		}
	}
	...
	*keywords = g_list_reverse(list);
	if (comment_build) {
		if (comment)
			*comment = g_strdup(comment_build->str);
...
GList *keyword_list_pull(GtkWidget *text_widget) {
	...
	while (*ptr != '\0') {
		...
		if (l > 0)
			list = g_list_append(list, g_strndup(begin, l));
	}
...
static void metadata_set_keywords(const gchar *path, GList *list, gint add) {
	...
	if (add) {
		...
		while (work) {
			...
			if (key)
				keywords = g_list_append(keywords, g_strdup(key));
		}
		save_list = keywords;
	}
	else {
...
gint bar_info_event(GtkWidget *bar, GdkEvent *event) {
	...
	if (GTK_WIDGET_HAS_FOCUS(bd->keyword_view))
		return gtk_widget_event(bd->keyword_view, event);
	if (GTK_WIDGET_HAS_FOCUS(bd->comment_view))
		return gtk_widget_event(bd->comment_view, event);
