...
gpointer submenu_item_get_data(GtkWidget *menu) {
	if (!menu->parent || !GTK_IS_MENU(menu->parent))
		return NULL;
...
GtkWidget *submenu_add_sort(GtkWidget *menu, GCallback func, gpointer data,
	...
	if (include_path)
		submenu_add_sort_item(submenu, parent, func, SORT_PATH, show_current, type);
	if (include_none)
		submenu_add_sort_item(submenu, parent, func, SORT_NONE, show_current, type);
