...
static void vdtree_busy_push(ViewDirTree *vdt) {
	if (vdt->busy_ref == 0)
		set_cursor(vdt->treeview, GDK_WATCH);
	vdt->busy_ref++;
}

static void vdtree_busy_pop(ViewDirTree *vdt) {
	if (vdt->busy_ref == 1)
		set_cursor(vdt->treeview, -1);
	if (vdt->busy_ref > 0)
		vdt->busy_ref--;
}
...
static gint vdtree_rename_row_cb(TreeEditData *td, const gchar *old, const gchar *new, gpointer data) {
	...
	store = gtk_tree_view_get_model(GTK_TREE_VIEW(vdt->treeview));
	if (!gtk_tree_model_get_iter(store, &iter, td->path))
		return FALSE;
...
static void vdtree_pop_menu_tree_cb(GtkWidget *widget, gpointer data) {
	...
	if (vdt->layout)
		layout_views_set(vdt->layout, FALSE, vdt->layout->icon_view);
}
...
static GtkTargetEntry vdtree_dnd_drop_types[] = { { "text/uri-list", 0, TARGET_URI_LIST }
};
...
static void vdtree_dnd_drop_expand_cancel(ViewDirTree *vdt) {
	if (vdt->drop_expand_id != -1)
		g_source_remove(vdt->drop_expand_id);
	vdt->drop_expand_id = -1;
}
...
static void vdtree_dnd_drop_scroll_cancel(ViewDirTree *vdt) {
	if (vdt->drop_scroll_id != -1)
		g_source_remove(vdt->drop_scroll_id);
...
static gint vdtree_auto_scroll_notify_cb(GtkWidget *widget, gint x, gint y, gpointer data) {
	...
	if (!vdt->drop_fd || vdt->drop_list)
		return FALSE;

	if (vdt->drop_scroll_id == -1)
		vdt->drop_scroll_id = g_idle_add(vdtree_auto_scroll_idle_cb, vdt);
...
static GList *parts_list(const gchar *path) {
	...
	while (*strp != '\0') {
		if (*strp == '/') {
			if (l > 0)
				list = g_list_prepend(list, g_strndup(strb, l));
			strp++;
			strb = strp;
			l = 0;
		}
...
static gint vdtree_find_iter_by_data(ViewDirTree *vdt, GtkTreeIter *parent, NodeData *nd, GtkTreeIter *iter) {
	...
	store = gtk_tree_view_get_model(GTK_TREE_VIEW(vdt->treeview));
	if (!nd || !gtk_tree_model_iter_children(store, iter, parent))
		return -1;
	do	{
		NodeData *cnd;
...
static NodeData *vdtree_find_iter_by_name(ViewDirTree *vdt, GtkTreeIter *parent, const gchar *name, GtkTreeIter *iter) {
	...
	store = gtk_tree_view_get_model(GTK_TREE_VIEW(vdt->treeview));
	if (!name || !gtk_tree_model_iter_children(store, iter, parent))
		return NULL;
	do	{
		NodeData *nd;
...
static gint vdtree_populate_path_by_iter(ViewDirTree *vdt, GtkTreeIter *iter, gint force, const gchar *target_path) {
	...
	if (nd->expanded) {
		if (!force && current_time - nd->last_update < 10)
			return TRUE;
		if (!isdir(nd->fd->path)) {
			if (vdt->click_fd == nd->fd)
				vdt->click_fd = NULL;
			if (vdt->drop_fd == nd->fd)
				vdt->drop_fd = NULL;
			gtk_tree_store_remove(GTK_TREE_STORE(store), iter);
			vdtree_node_free(nd);
			return FALSE;
		}
		if (!force && filetime(nd->fd->path) == nd->fd->date)
			return TRUE;
	}
	...
	while (work) {
		...
		if (vdt->click_fd == cnd->fd)
			vdt->click_fd = NULL;
		if (vdt->drop_fd == cnd->fd)
			vdt->drop_fd = NULL;
...
static void vdtree_select_row(ViewDirTree *vdt, FileData *fd) {
	...
	if (!vdtree_populate_path_by_iter(vdt, &iter, FALSE, vdt->path))
		return;
...
gint vdtree_set_path(ViewDirTree *vdt, const gchar *path) {
	...
	if (!path)
		return FALSE;
	if (vdt->path && strcmp(path, vdt->path) == 0)
		return TRUE;
...
static void vdtree_menu_position_cb(GtkMenu *menu, gint *x, gint *y, gboolean *push_in, gpointer data) {
	...
	if (vdtree_find_row(vdt, vdt->click_fd, &iter, NULL) < 0)
		return;
	store = gtk_tree_view_get_model(GTK_TREE_VIEW(vdt->treeview));
	tpath = gtk_tree_model_get_path(store, &iter);
...
static gint vdtree_clicked_on_expander(GtkTreeView *treeview, GtkTreePath *tpath,
	...
	if (column != gtk_tree_view_get_expander_column(treeview))
		return FALSE;
	...
	if (x <= depth * exp_width) {
		if (left_of_expander) *left_of_expander = !(x >= (depth - 1)
			* exp_width);
		return TRUE;
	}
