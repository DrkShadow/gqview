...
static gint pan_queue_step(PanWindow *pw) {
	...
	if (pi->type == PAN_ITEM_IMAGE) {
		...
		if (thumb_loader_start(pw->tl, pi->fd->path))
			return FALSE;
...
void pan_layout_resize(PanWindow *pw) {
	...
	while (work) {
		...
		if (width < pi->x + pi->width)
			width = pi->x + pi->width;
		if (height < pi->y + pi->height)
			height = pi->y + pi->height;
	}
	work = pw->list_static;
	while (work) {
		PanItem *pi;
	...
	if (width < pr->window_width)
		width = pr->window_width;
	if (height < pr->window_width)
		height = pr->window_height;
...
static const gchar *pan_menu_click_path(PanWindow *pw) {
	if (pw->click_pi && pw->click_pi->fd)
		return pw->click_pi->fd->path;
	return NULL;
}
...
static gint pan_window_key_press_cb(GtkWidget *widget, GdkEventKey *event, gpointer data) {
	...
	if (event->state & GDK_CONTROL_MASK) {
		...
		switch (event->keyval) {
			...
			case 'C': case 'c':
				if (path)
					file_util_copy(path, NULL, NULL, GTK_WIDGET(pr));
				break;
			case 'M': case 'm':
				if (path)
					file_util_move(path, NULL, NULL, GTK_WIDGET(pr));
				break;
			case 'R': case 'r':
				if (path)
					file_util_rename(path, NULL, GTK_WIDGET(pr));
				break;
			case 'D': case 'd':
				if (path)
					file_util_delete(path, NULL, GTK_WIDGET(pr));
				break;
			case 'P': case 'p':
				if (path)
					info_window_new(path, NULL);
				break;
			case 'F': case 'f':
...
static void pan_info_add_exif(PanTextAlignment *ta, FileData *fd) {
	...
	if (work)
		pan_text_alignment_add(ta, "---", NULL);
	while (work) {
		const gchar *name;
		gchar *label;
		gchar *text;
...
static gint pan_search_by_date(PanWindow *pw, const gchar *text) {
	...
	ptr = (gchar *)text;
	while (*ptr != '\0') {
		if (!g_unichar_isdigit(*ptr) && !valid_date_separator(*ptr))
			return FALSE;
		ptr++;
	}
...
static void pan_search_toggle_cb(GtkWidget *button, gpointer data) {
	...
	if (gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(button)) == visible)
		return;
...
static void pan_copy_cb(GtkWidget *widget, gpointer data) {
	...
	path = pan_menu_click_path(pw);
	if (path)
		file_util_copy(path, NULL, NULL, pw->imd->widget);
}
...
static void pan_move_cb(GtkWidget *widget, gpointer data) {
	...
	path = pan_menu_click_path(pw);
	if (path)
		file_util_move(path, NULL, NULL, pw->imd->widget);
}
...
static void pan_rename_cb(GtkWidget *widget, gpointer data) {
	...
	path = pan_menu_click_path(pw);
	if (path)
		file_util_rename(path, NULL, pw->imd->widget);
}
...
static void pan_delete_cb(GtkWidget *widget, gpointer data) {
	...
	path = pan_menu_click_path(pw);
	if (path)
		file_util_delete(path, NULL, pw->imd->widget);
}
...
static void pan_window_get_dnd_data(GtkWidget *widget, GdkDragContext *context,
	...
	if (gtk_drag_get_source_widget(context) == pw->imd->pr)
		return;
