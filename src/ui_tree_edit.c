...
gint tree_view_row_get_visibility(GtkTreeView *widget, GtkTreeIter *iter, gint fully_visible) {
	...
	if (fully_visible) {
		if (cy < vrect.y)
			return -1;
		if (cy + crect.height > vrect.y + vrect.height)
			return 1;
		return 0;
	}

	if (cy + crect.height < vrect.y)
		return -1;
	if (cy > vrect.y + vrect.height)
		return 1;
	return 0;
}
...
gint widget_auto_scroll_start(GtkWidget *widget, GtkAdjustment *v_adj, gint scroll_speed, gint region_size,
	...
	if (!widget || !v_adj)
		return 0;
	if (g_object_get_data(G_OBJECT(widget), "autoscroll"))
		return 0;
	if (scroll_speed < 1)
		scroll_speed = AUTO_SCROLL_DEFAULT_SPEED;
	if (region_size < 1)
		region_size = AUTO_SCROLL_DEFAULT_REGION;
