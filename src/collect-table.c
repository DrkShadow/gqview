...
static gint collection_table_find_iter(CollectTable *ct, CollectInfo *info, GtkTreeIter *iter, gint *column) {
	...
	if (!collection_table_find_position(ct, info, &row, &col))
		return FALSE;
	if (!gtk_tree_model_iter_nth_child(store, iter, NULL, row))
		return FALSE;
	if (column)
		*column = col;
...
static gint collection_table_get_icon_width(CollectTable *ct) {
	...
	if (width < THUMB_MIN_ICON_WIDTH)
		width = THUMB_MIN_ICON_WIDTH;
	if (width > THUMB_MAX_ICON_WIDTH)
...
static void collection_table_selection_set(CollectTable *ct, CollectInfo *info, SelectionType value, GtkTreeIter *iter) {
	...
	if (iter) {
		...
		if (collection_table_find_iter(ct, info, &row, NULL)) {
			gtk_tree_model_get(store, &row, CTABLE_COLUMN_POINTER, &list, -1);
			if (list)
				gtk_list_store_set(GTK_LIST_STORE(store), &row, CTABLE_COLUMN_POINTER, list, -1);
		}
	}
}
...
static void tip_show(CollectTable *ct) {
	...
	if (!GTK_WIDGET_REALIZED(ct->tip_window))
		gtk_widget_realize(ct->tip_window);
...
static void tip_hide(CollectTable *ct) {
	if (ct->tip_window)
		gtk_widget_destroy(ct->tip_window);
	ct->tip_window = NULL;
}
...
static void collection_table_menu_pos_cb(GtkMenu *menu, gint *x, gint *y, gboolean *push_in, gpointer data) {
	...
	if (!collection_table_find_iter(ct, ct->click_info, &iter, &column))
		return;
	store = gtk_tree_view_get_model(GTK_TREE_VIEW(ct->listview));
	tpath = gtk_tree_model_get_path(store, &iter);
...
static void collection_table_insert_marker(CollectTable *ct, CollectInfo *info, gint enable) {
	...
	if (!enable) {
		if (ct->marker_window)
			gdk_window_destroy(ct->marker_window);
	...
	if (info) {
		...
		if (!gdk_window_is_visible(ct->marker_window))
			gdk_window_show(ct->marker_window);
	}
	else {
		if (gdk_window_is_visible(ct->marker_window))
			gdk_window_hide(ct->marker_window);
	}
}
...
static void collection_table_populate(CollectTable *ct, gint resize) {
	...
	if (resize) {
		...
		if (GTK_WIDGET_REALIZED(ct->listview))
			gtk_tree_view_columns_autosize(GTK_TREE_VIEW(ct->listview));
	}
...
static void collection_table_populate_at_new_size(CollectTable *ct, gint w, gint h, gint force) {
	...
	if (debug)
		printf("col tab pop cols=%d rows=%d\n", ct->columns, ct->rows);
}
...
static void collection_table_move_by_info_list(CollectTable *ct, GList *info_list, gint row, gint col) {
	...
	if (!info_list->next && info_list->data == info)
		return;

	if (info)
		insert_pos = g_list_find(ct->cd->list, info);
...
static void confirm_dir_list_do(CollectTable *ct, GList *list, gint recursive) {
	...
	while (work) {
		...
		work = work->next;
		if (isdir(path))
			collection_table_add_dir_recursive(ct, path, recursive);
	}
...
static GtkTargetEntry collection_drag_types[] = { { "application/x-gqview-collection-member", 0, TARGET_APP_COLLECTION_MEMBER },
	{ "text/uri-list", 0, TARGET_URI_LIST }, { "text/plain", 0, TARGET_TEXT_PLAIN }
};
static gint n_collection_drag_types = 3;

static GtkTargetEntry collection_drop_types[] = { { "application/x-gqview-collection-member", 0, TARGET_APP_COLLECTION_MEMBER },
