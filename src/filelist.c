...
void filter_add_defaults(void) {
	...
	filter_add_if_missing("dng", "Adobe Digital Negative", ".dng", TRUE);
}
...
gint filter_name_exists(const gchar *name) {
	GList *work;
	if (!extension_list || file_filter_disable)
		return TRUE;
...
void filter_parse(const gchar *text) {
	...
	if (key && strlen(key) > 0 && ext)
		filter_add(key, desc, ext, enabled);
...
GList *path_list_filter(GList *list, gint is_dir_list) {
	...
	if (!is_dir_list && file_filter_disable && show_dot_files)
		return list;
