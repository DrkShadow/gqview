...
void pan_item_size_coordinates(PanItem *pi, gint border, gint *w, gint *h) {
	...
	if (*w < pi->x + pi->width + border)
		*w = pi->x + pi->width + border;
	if (*h < pi->y + pi->height + border)
		*h = pi->y + pi->height + border;
}
...
void pan_item_tri_border(PanItem *pi, gint borders,
			 guint8 r, guint8 g, guint8 b, guint8 a) {
	if (!pi || pi->type != PAN_ITEM_TRIANGLE)
		return;
		...
		if ((pi->type == type || type == PAN_ITEM_NONE) && pi->fd) {
			...
			if (path[0] == '/') {
				...
				if (partial) {
					...
					if (strcasecmp(path, pi->fd->name) == 0)
						match = TRUE;
				}
				else {
