...
static gint pr_parent_window_sizable(PixbufRenderer *pr) {
	...
	if (!pr->window_fit)
		return FALSE;
	if (!GTK_WIDGET(pr)->window)
		return FALSE;

	if (!pr->parent_window->window)
		return FALSE;
...
static gint pr_parent_window_resize(PixbufRenderer *pr, gint w, gint h) {
	...
	if (!pr_parent_window_sizable(pr))
		return FALSE;
...
static void pr_overlay_get_position(PixbufRenderer *pr, OverlayData *od,
	...
	if (od->relative) {
		if (px < 0)
			px = pr->window_width - pw + px;
		if (py < 0)
			py = pr->window_height - ph + py;
	}
	...
	if (abs(x) < PR_SCROLLER_DEAD_ZONE * PR_SCROLLER_UPDATES_PER_SEC) {
		...
		if (x >= 0) {
			...
			if (x < xinc)
				xinc = x;
			if (x > xinc)
				xinc = MIN(xinc + x / PR_SCROLLER_UPDATES_PER_SEC, x);
		}
		else {
			if (xinc > 0)
				xinc = 0;
			if (x > xinc)
				xinc = x;
			if (x < xinc)
				xinc = MAX(xinc + x / PR_SCROLLER_UPDATES_PER_SEC, x);
		}
	}
	...
	if (abs(y) < PR_SCROLLER_DEAD_ZONE * PR_SCROLLER_UPDATES_PER_SEC) {
		...
		if (y >= 0) {
			...
			if (y < yinc)
				yinc = y;
			if (y > yinc)
				yinc = MIN(yinc + y / PR_SCROLLER_UPDATES_PER_SEC, y);
		}
		else {
			if (yinc > 0)
				yinc = 0;
			if (y > yinc)
				yinc = y;
			if (y < yinc)
				yinc = MAX(yinc + y / PR_SCROLLER_UPDATES_PER_SEC, y);
		}
	}
...
static gint pr_source_tile_render(PixbufRenderer *pr, ImageTile *it,
	...
	if (pr->zoom == 1.0 || pr->scale == 1.0) {
		...
		if (pr->image_width == 0 || pr->image_height == 0)
			return FALSE;
		...
		if (pr->width < PR_MIN_SCALE_SIZE || pr->height < PR_MIN_SCALE_SIZE)
			fast = TRUE;
...
void pixbuf_renderer_set_tiles_size(PixbufRenderer *pr, gint width, gint height) {
	...
	if (!pr->source_tiles_enabled)
		return;
	if (pr->image_width == width && pr->image_height == height)
		return;
...
static void pr_tile_free(ImageTile *it) {
	...
	if (it->pixbuf)
		gdk_pixbuf_unref(it->pixbuf);
	if (it->pixmap)
		g_object_unref(it->pixmap);
...
static ImageTile *pr_tile_add(PixbufRenderer *pr, gint x, gint y) {
	...
	if (it->x + it->w > pr->width)
		it->w = pr->width - it->x;
	if (it->y + it->h > pr->height)
		it->h = pr->height - it->y;
...
static void pr_tile_render(PixbufRenderer *pr, ImageTile *it,
	...
	if (it->render_todo == TILE_RENDER_NONE && it->pixmap && !new_data)
		return;
...
static gint pr_queue_to_tiles(PixbufRenderer *pr, gint x, gint y, gint w, gint h,
	...
	if (clamp && !pr_clamp_to_visible(pr, &x, &y, &w, &h))
		return FALSE;
	...
	for (j = y1; j <= y2; j += pr->tile_height) {
		for (i = x1; i <= x2; i += pr->tile_width) {
			...
			if (it) {
				...
				qd->w = x + w - i - qd->x;
				if (qd->x + qd->w > pr->tile_width)
					qd->w = pr->tile_width - qd->x;
				...
				qd->h = y + h - j - qd->y;
				if (qd->y + qd->h > pr->tile_height)
					qd->h = pr->tile_height - qd->y;
...
static void pr_zoom_sync(PixbufRenderer *pr, gdouble zoom,
	...
	if (!pr_zoom_clamp(pr, zoom, force, new, force, &redrawn))
		return;
...
void pixbuf_renderer_scroll(PixbufRenderer *pr, gint x, gint y) {
	...
	pr_scroll_clamp(pr);
	if (pr->x_scroll == old_x && pr->y_scroll == old_y)
		return;
...
static gint pr_mouse_motion_cb(GtkWidget *widget, GdkEventButton *bevent, gpointer data) {
	...
	if (!pr->in_drag || !gdk_pointer_is_grabbed())
		return FALSE;
