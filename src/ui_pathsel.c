...
static GtkTargetEntry dest_drag_types[] = { { "text/uri-list", 0, TARGET_URI_LIST },
...
static void dest_view_delete(Dest_Data *dd, GtkTreeView *view) {
	...
	if (view != GTK_TREE_VIEW(dd->f_view))
		return;
	if (!dd->right_click_path)
		return;
...
static void dest_new_dir_cb(GtkWidget *widget, gpointer data) {
	...
	if (!mkdir_utf8(path, 0755)) {
		...
		if (from_text)
			gtk_entry_set_text(GTK_ENTRY(dd->entry), dd->path);
...
static void dest_entry_changed_cb(GtkEditable *editable, gpointer data) {
	...
	if (dd->path && strcmp(path, dd->path) == 0)
		return;
...
static void dest_filter_add(Dest_Data *dd, const gchar *filter, const gchar *description, gint set) {
	...
	while (work) {
		...
		if (strcmp(f, filter) == 0) {
			if (set)
				gtk_combo_box_set_active(GTK_COMBO_BOX(dd->filter_combo), c);
			return;
		}
		work = work->next;
		c++;
	...
	if (set)
		gtk_entry_set_text(GTK_ENTRY(GTK_BIN(dd->filter_combo)->child), filter);
...
GtkWidget *path_selection_new_with_files(GtkWidget *entry, const gchar *path,
	...
	if (path && path[0] == '/' && isdir(path)) {
		...
		if (buf && buf[0] == '/' && isdir(buf)) {
			...
			if (path)
				gtk_editable_insert_text(GTK_EDITABLE(dd->entry), "/", -1, &pos);
			if (path)
				gtk_editable_insert_text(GTK_EDITABLE(dd->entry), path, -1, &pos);
		}
		g_free(buf);
	}
