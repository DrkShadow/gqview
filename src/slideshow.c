...
void slideshow_free(SlideShowData *ss) {
	...
	if (ss->stop_func)
		ss->stop_func(ss, ss->stop_data);

	if (ss->path_list)
		path_list_free(ss->path_list);
	if (ss->cd)
		collection_unref(ss->cd);
...
gint slideshow_should_continue(SlideShowData *ss) {
	...
	if (path && ss->layout_path &&
		...
		if (ss->from_selection && ss->slide_count == layout_selection_count(ss->layout, NULL))
			return TRUE;
		if (!ss->from_selection && ss->slide_count == layout_list_count(ss->layout, NULL))
			return TRUE;
	}
...
static gint slideshow_step(SlideShowData *ss, gint forward) {
	...
	if (enable_read_ahead) {
		...
		if (ss->path_list) {
			...
			if (info)
				image_prebuffer_set(ss->imd, info->path);
		}
...
static SlideShowData *real_slideshow_start(ImageWindow *imd, LayoutWindow *lw,
	...
	if (!path_list && !cd && layout_list_count(lw, NULL) < 1)
		return NULL;
