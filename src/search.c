...
static const MatchList text_search_menu_path[] = { { N_("folder"),		SEARCH_MATCH_NONE },
	{ N_("comments"),	SEARCH_MATCH_ALL }, { N_("results"),	SEARCH_MATCH_CONTAINS }
};

static const MatchList text_search_menu_name[] = { { N_("contains"),	SEARCH_MATCH_CONTAINS },
	{ N_("is"),		SEARCH_MATCH_EQUAL }
};

static const MatchList text_search_menu_size[] = { { N_("equal to"),	SEARCH_MATCH_EQUAL },
	{ N_("less than"),	SEARCH_MATCH_UNDER }, { N_("greater than"),	SEARCH_MATCH_OVER },
	{ N_("between"),	SEARCH_MATCH_BETWEEN }
};

static const MatchList text_search_menu_date[] = { { N_("equal to"),	SEARCH_MATCH_EQUAL },
	{ N_("before"),		SEARCH_MATCH_UNDER }, { N_("after"),		SEARCH_MATCH_OVER },
	{ N_("between"),	SEARCH_MATCH_BETWEEN }
};

static const MatchList text_search_menu_keyword[] = { { N_("match all"),	SEARCH_MATCH_ALL },
	{ N_("match any"),	SEARCH_MATCH_ANY }, { N_("exclude"),	SEARCH_MATCH_NONE }
};
...
static gint search_result_selection_util(SearchData *sd, gint64 *bytes, GList **list) {
	...
	while (work) {
		...
		if (bytes || list) {
			...
			if (list)
				plist = g_list_prepend(plist, g_strdup(fd->path));
		}
...
static void search_result_thumb_set(SearchData *sd, FileData *fd, GtkTreeIter *iter) {
	...
	if (iter)
		gtk_list_store_set(store, iter, SEARCH_COLUMN_THUMB, fd->pixbuf, -1);
}

static void search_result_thumb_do(SearchData *sd) {
	FileData *fd;

	if (!sd->thumb_loader || !sd->thumb_fd)
		return;
	fd = sd->thumb_fd;
...
static void search_result_thumb_step(SearchData *sd) {
	...
	while (!fd && valid) {
		...
		if (pixbuf || fd->pixbuf) {
			if (!pixbuf)
				gtk_list_store_set(GTK_LIST_STORE(store), &iter, SEARCH_COLUMN_THUMB, fd->pixbuf, -1);
			row++;
			fd = NULL;
		}
		valid = gtk_tree_model_iter_next(store, &iter);
...
static void search_result_thumb_enable(SearchData *sd, gint enable) {
	...
	if (!sd->search_folder_list && !sd->search_file_list)
		search_result_thumb_step(sd);
}
...
static void sr_menu_view_cb(GtkWidget *widget, gpointer data) {
	...
	if (sd->click_fd)
		layout_image_set_path(NULL, sd->click_fd->path);
}
...
static GtkTargetEntry result_drag_types[] = { { "text/uri-list", 0, TARGET_URI_LIST },
...
static gint search_file_do_extra(SearchData *sd, FileData *fd, gint *match,
	...
	if (sd->img_cd->dimensions) {
		if (width)
			*width = sd->img_cd->width;
		if (height)
			*height = sd->img_cd->height;
	}
...
static gint search_step_cb(gpointer data) {
	...
	if (g_list_find(sd->search_done_list, fd) == NULL) {
		...
		if (sd->search_type == SEARCH_MATCH_NONE) {
			...
			if (path != fd->path)
				success = filelist_read(path, &list, NULL);
...
static void search_start_cb(GtkWidget *widget, gpointer data) {
	...
	if (sd->match_name_enable)
		history_combo_append_history(sd->entry_name, NULL);
	g_free(sd->search_name);
...
static gint search_result_sort_cb(GtkTreeModel *model, GtkTreeIter *a, GtkTreeIter *b, gpointer data) {
	...
	switch (n) {
		case SEARCH_COLUMN_RANK:
			if (((MatchFileData *)fda)->rank > ((MatchFileData *)fdb)->rank)
				return 1;
			if (((MatchFileData *)fda)->rank < ((MatchFileData *)fdb)->rank)
				return -1;
			return 0;
...
static GtkWidget *menu_choice(GtkWidget *box, GtkWidget **check, GtkWidget **menu,
	...
	if (check)
		*check = button;
	if (value)
		g_object_set_data(G_OBJECT(button), "check_var", value);
