...
static void collection_load_thumb_do(CollectionData *cd) {
	...
	if (!cd->thumb_loader || !g_list_find(cd->list, cd->thumb_info))
		return;
	...
	if (cd->info_updated_func)
		cd->info_updated_func(cd, cd->thumb_info, cd->info_updated_data);
}
...
static void collection_load_thumb_step(CollectionData *cd) {
	...
	if (!ci || ci->pixbuf) {
		...
		if (cd->info_updated_func)
			cd->info_updated_func(cd, NULL, cd->info_updated_data);
...
gint collection_load_begin(CollectionData *cd, const gchar *path, gint append) {
	if (!collection_load(cd, path, append))
		return FALSE;
...
static gint collect_manager_process_entry(CollectManagerEntry *entry) {
	...
	if (success && cd->changed) {
		...
		if (debug)
			printf("collection manager updated: %s\n", entry->path);
	}
	collection_unref(cd);
...
static gint collect_manager_process_cb(gpointer data) {
	...
	if (debug)
		printf("collection manager is up to date\n");
	return FALSE;
}

static gint collect_manager_timer_cb(gpointer data) {
	if (debug)
		printf("collection manager timer expired\n");
...
static void collect_manager_timer_push(gint stop) {
	...
	if (!stop) {
		...
		if (debug)
			printf("collection manager timer started\n");
	}
}
...
void collect_manager_flush(void) {
	...
	if (debug)
		printf("collection manager flushing\n");
