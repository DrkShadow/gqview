...
static gint sizer_motion_cb(GtkWidget *widget, GdkEventButton *bevent, gpointer data) {
	...
	if (sd->hsize_min >= 0)
		w = MAX(w, sd->hsize_min);
	if (sd->vsize_min >= 0)
		h = MAX(h, sd->vsize_min);
	...
	if (sd->bounding_widget) {
		...
		if (w < sd->sizer->allocation.width)
			w = sd->sizer->allocation.width;
		if (h < sd->sizer->allocation.height)
			h = sd->sizer->allocation.height;
	}

	if (sd->hsize_max >= 0)
		w = MIN(w, sd->hsize_max);
	if (sd->vsize_max >= 0)
		h = MIN(h, sd->vsize_max);

	if (w == sd->parent->allocation.width)
		w = -1;
	if (h == sd->parent->allocation.height)
		h = -1;

	if (w > 0 || h > 0)
		gtk_widget_set_size_request(sd->parent, w, h);
