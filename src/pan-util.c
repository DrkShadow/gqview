...
gint pan_date_compare(time_t a, time_t b, PanDateLengthType length) {
	...
	if (length == PAN_DATE_LENGTH_EXACT)
		return (a == b);
	...
	if (ta.tm_year != tb.tm_year)
		return FALSE;
	if (length == PAN_DATE_LENGTH_YEAR)
		return TRUE;

	if (ta.tm_mon != tb.tm_mon)
		return FALSE;
	if (length == PAN_DATE_LENGTH_MONTH)
		return TRUE;
