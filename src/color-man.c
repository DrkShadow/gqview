...
static void color_man_cache_unref(ColorManCache *cc) {
	...
	if (cc->refcount < 1) {
		if (cc->transform)
			cmsDeleteTransform(cc->transform);
		if (cc->profile_in)
			cmsCloseProfile(cc->profile_in);
		if (cc->profile_out)
			cmsCloseProfile(cc->profile_out);
...
static ColorManCache *color_man_cache_new(ColorManProfileType in_type, const gchar *in_file,
	...
	if (!cc->transform) {
		if (debug)
			printf("failed to create color profile transform\n");
