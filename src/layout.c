...
static void layout_color_menu_input_cb(GtkWidget *widget, gpointer data) {
	...
	if (type < 0 || type > COLOR_PROFILE_INPUTS)
		return;
...
static void layout_color_button_press_cb(GtkWidget *widget, gpointer data) {
	...
	for (i = 0; i < COLOR_PROFILE_INPUTS; i++) {
		...
		if (!name)
			name = filename_from_path(color_profile_input_file[i]);
...
static GtkWidget *layout_status_label(gchar *text, GtkWidget *box, gint start, gint size, gint expand) {
	...
	if (size)
		gtk_widget_set_size_request(frame, size, -1);
...
static void layout_status_setup(LayoutWindow *lw, GtkWidget *box, gint small_format) {
	...
	if (small_format)
		gtk_box_pack_end(GTK_BOX(hbox), lw->info_color, FALSE, FALSE, 0);
	...
	if (!small_format)
		gtk_box_pack_start(GTK_BOX(hbox), lw->info_color, FALSE, FALSE, 0);
...
#if 0
	if (lw->vfl)
		vflist_scroll_to_subpart(lw->vfl, needle);
	if (lw->vfi)
		vficon_scroll_to_subpart(lw->vfi, needle);
#endif
}
...
GList *layout_list(LayoutWindow *lw) {
	...
	if (lw->vfl)
		return vflist_get_list(lw->vfl);
	if (lw->vfi)
		return vficon_get_list(lw->vfi);
...
gint layout_list_count(LayoutWindow *lw, gint64 *bytes) {
	...
	if (lw->vfl)
		return vflist_count(lw->vfl, bytes);
	if (lw->vfi)
		return vficon_count(lw->vfi, bytes);
...
const gchar *layout_list_get_path(LayoutWindow *lw, gint index) {
	...
	if (lw->vfl)
		return vflist_index_get_path(lw->vfl, index);
	if (lw->vfi)
		return vficon_index_get_path(lw->vfi, index);
...
gint layout_list_get_index(LayoutWindow *lw, const gchar *path) {
	...
	if (lw->vfl)
		return vflist_index_by_path(lw->vfl, path);
	if (lw->vfi)
		return vficon_index_by_path(lw->vfi, path);
...
void layout_list_sync_path(LayoutWindow *lw, const gchar *path) {
	...
	if (lw->vfl)
		vflist_select_by_path(lw->vfl, path);
	if (lw->vfi)
		vficon_select_by_path(lw->vfi, path);
}
...
static void layout_list_sync_sort(LayoutWindow *lw) {
	...
	if (lw->vfl)
		vflist_sort_set(lw->vfl, lw->sort_method, lw->sort_ascend);
	if (lw->vfi)
		vficon_sort_set(lw->vfi, lw->sort_method, lw->sort_ascend);
}
...
GList *layout_selection_list(LayoutWindow *lw) {
	...
	if (layout_image_get_collection(lw, NULL)) {
		...
		if (path)
			return g_list_append(NULL, g_strdup(path));
		return NULL;
	}

	if (lw->vfl)
		return vflist_selection_get_list(lw->vfl);
	if (lw->vfi)
		return vficon_selection_get_list(lw->vfi);
...
GList *layout_selection_list_by_index(LayoutWindow *lw) {
	...
	if (lw->vfl)
		return vflist_selection_get_list_by_index(lw->vfl);
	if (lw->vfi)
		return vficon_selection_get_list_by_index(lw->vfi);
...
gint layout_selection_count(LayoutWindow *lw, gint64 *bytes) {
	...
	if (lw->vfl)
		return vflist_selection_count(lw->vfl, bytes);
	if (lw->vfi)
		return vficon_selection_count(lw->vfi, bytes);
...
void layout_select_all(LayoutWindow *lw) {
	...
	if (lw->vfl)
		vflist_select_all(lw->vfl);
	if (lw->vfi)
		vficon_select_all(lw->vfi);
}
...
void layout_select_none(LayoutWindow *lw) {
	...
	if (lw->vfl)
		vflist_select_none(lw->vfl);
	if (lw->vfi)
		vficon_select_none(lw->vfi);
}
...
static void layout_sync_path(LayoutWindow *lw) {
	...
	if (lw->vdl)
		vdlist_set_path(lw->vdl, lw->path);
	if (lw->vdt)
		vdtree_set_path(lw->vdt, lw->path);

	if (lw->vfl)
		vflist_set_path(lw->vfl, lw->path);
	if (lw->vfi)
		vficon_set_path(lw->vfi, lw->path);
}
...
gint layout_set_path(LayoutWindow *lw, const gchar *path) {
	...
	if (lw->path_entry)
		tab_completion_append_to_history(lw->path_entry, lw->path);
	layout_sync_path(lw);
...
static void layout_refresh_lists(LayoutWindow *lw) {
	...
	if (lw->vdl)
		vdlist_refresh(lw->vdl);
	if (lw->vdt)
		vdtree_refresh(lw->vdt);

	if (lw->vfl)
		vflist_refresh(lw->vfl);
	if (lw->vfi)
		vficon_refresh(lw->vfi);
}
...
static gint layout_check_for_update_cb(gpointer data) {
	...
	if (lw->path) {
		...
		if (new_time > 0 && new_time > lw->last_time) {
			if (debug)
				printf("layout path time changed, refreshing...\n");
...
void layout_sort_set(LayoutWindow *lw, SortType type, gint ascend) {
	if (!layout_valid(&lw))
		return;
	if (lw->sort_method == type && lw->sort_ascend == ascend)
		return;
...
void layout_views_set(LayoutWindow *lw, gint tree, gint icons) {
	...
	if (lw->tree_view == tree && lw->icon_view == icons)
		return;
...
static LayoutLocation layout_grid_compass(LayoutWindow *lw) {
	if (layout_location_single(lw->dir_location))
		return lw->dir_location;
	if (layout_location_single(lw->file_location))
		return lw->file_location;
...
void layout_colors_update(void) {
	...
	while (work) {
		...
		if (lw->image)
			image_background_set_black(lw->image, black_window_background);
	}
}
...
void layout_tools_float_set(LayoutWindow *lw, gint popped, gint hidden) {
	...
	if (lw->tools_float == popped && lw->tools_hidden == hidden)
		return;
...
static void layout_real_renamed(LayoutWindow *lw, const gchar *source, const gchar *dest) {
	...
	if (lw->vfl)
		update |= vflist_maint_renamed(lw->vfl, source, dest);
	if (lw->vfi)
		update |= vficon_maint_renamed(lw->vfi, source, dest);
...
static void layout_real_removed(LayoutWindow *lw, const gchar *path, GList *ignore_list) {
	...
	if (lw->image)
		layout_image_maint_removed(lw, path);

	if (lw->vfl)
		update |= vflist_maint_removed(lw->vfl, path, ignore_list);
	if (lw->vfi)
		update |= vficon_maint_removed(lw->vfi, path, ignore_list);
...
static void layout_real_moved(LayoutWindow *lw, const gchar *source, const gchar *dest, GList *ignore_list) {
	...
	if (lw->image)
		layout_image_maint_moved(lw, source, dest);

	if (lw->vfl)
		update |= vflist_maint_moved(lw->vfl, source, dest, ignore_list);
	if (lw->vfi)
		update |= vficon_maint_moved(lw->vfi, source, dest, ignore_list);
