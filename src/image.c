...
static void image_update_util(ImageWindow *imd) {
	if (imd->func_update)
		imd->func_update(imd, imd->data_update);
}
...
static void image_complete_util(ImageWindow *imd, gint preload) {
	if (imd->il && image_get_pixbuf(imd) != image_loader_get_pixbuf(imd->il))
		return;
	...
	if (!preload)
		imd->completed = TRUE;
	if (imd->func_complete)
		imd->func_complete(imd, preload, imd->data_complete);
}
...
static void image_state_set(ImageWindow *imd, ImageState state) {
	...
	if (imd->func_state)
		imd->func_state(imd, state, imd->data_state);
}
...
static void image_alter_real(ImageWindow *imd, AlterType type, gint clamp) {
	...
	if (exif_rotate)
		image_state_set(imd, IMAGE_STATE_ROTATE_AUTO);
}
	...
	if (imd->color_profile_use_image && exif) {
		...
		if (!item) {
			...
			if (exif_get_integer(exif, "ColorSpace", &cs) &&
				...
				if (debug)
					printf("Found EXIF ColorSpace of sRGB\n");
			}
		}
	}
	if (item && item->format == EXIF_FORMAT_UNDEFINED) {
		if (debug)
			printf("Found embedded color profile\n");
...
static void image_post_process(ImageWindow *imd, gint clamp) {
	...
	if (!imd->cm)
		image_post_process_alter(imd, clamp);
...
static void image_read_ahead_cancel(ImageWindow *imd) {
	if (debug)
		printf("read ahead cancelled for :%s\n", imd->read_ahead_path);
...
static void image_read_ahead_start(ImageWindow *imd) {
	/* already started ? */
	if (!imd->read_ahead_path || imd->read_ahead_il || imd->read_ahead_pixbuf)
		return;
	...
	if (debug)
		printf("read ahead started for :%s\n", imd->read_ahead_path);
...
static void image_read_ahead_set(ImageWindow *imd, const gchar *path) {
	if (imd->read_ahead_path && path && strcmp(imd->read_ahead_path, path) == 0)
		return;
...
static void image_post_buffer_set(ImageWindow *imd, const gchar *path, GdkPixbuf *pixbuf, gint color_row) {
	...
	if (imd->prev_pixbuf)
		g_object_unref(imd->prev_pixbuf);
	...
	if (imd->prev_pixbuf &&
		...
		if (imd->prev_color_row >= 0) {
			...
			if (imd->color_profile_use_image)
				exif = exif_read(imd->image_path, TRUE);
...
static void image_load_pixbuf_ready(ImageWindow *imd) {
	if (image_get_pixbuf(imd) || !imd->il)
		return;
...
static void image_load_buffer_throttle(ImageLoader *il) {
	if (!il || il->bytes_total < IMAGE_THROTTLE_THRESHOLD)
		return;
...
static gint image_load_begin(ImageWindow *imd, const gchar *path) {
	...
	if (image_post_buffer_get(imd)) {
		if (debug)
			printf("from post buffer: %s\n", imd->image_path);
		return TRUE;
	}

	if (image_read_ahead_check(imd)) {
		if (debug)
			printf("from read ahead buffer: %s\n", imd->image_path);
		return TRUE;
	}
...
void image_change_from_collection(ImageWindow *imd, CollectionData *cd, CollectInfo *info, gdouble zoom) {
	if (!cd || !info || !g_list_find(cd->list, info))
		return;
...
void image_change_from_image(ImageWindow *imd, ImageWindow *source) {
	...
	if (imd->read_ahead_il)
		image_loader_sync_data(imd->read_ahead_il, imd);
...
void image_reload(ImageWindow *imd) {
	if (pixbuf_renderer_get_tiles((PixbufRenderer *)imd->pr))
		return;
...
void image_auto_refresh(ImageWindow *imd, gint interval) {
	...
	if (interval == 0)
		interval = IMAGE_AUTO_REFRESH_TIME;
...
gint image_color_profile_get(ImageWindow *imd,
	...
	if (screen_type)
		*screen_type = imd->color_profile_screen;
	if (use_image)
...
void image_color_profile_set_use(ImageWindow *imd, gint enable) {
	...
	if (imd->color_profile_enable == enable)
		return;
...
void image_to_root_window(ImageWindow *imd, gint scaled) {
	...
	if (gdk_drawable_get_visual(rootwindow) != gdk_visual_get_system())
		return;
