...
static PixbufInline inline_pixbuf_data[] = { { PIXBUF_INLINE_FOLDER_CLOSED,	folder_closed },
	{ PIXBUF_INLINE_FOLDER_LOCKED,	folder_locked }, { PIXBUF_INLINE_FOLDER_OPEN,	folder_open },
	{ PIXBUF_INLINE_FOLDER_UP,	folder_up }, { PIXBUF_INLINE_SCROLLER,	icon_scroller },
	{ PIXBUF_INLINE_BROKEN,		icon_broken }, { PIXBUF_INLINE_ICON,		gqview_icon },
	{ PIXBUF_INLINE_LOGO,		gqview_logo }, { PIXBUF_INLINE_ICON_FLOAT,	icon_float },
	{ PIXBUF_INLINE_ICON_THUMB,	icon_thumb }, { PIXBUF_INLINE_ICON_BOOK,	icon_book },
	{ PIXBUF_INLINE_ICON_CONFIG,	icon_config }, { PIXBUF_INLINE_ICON_TOOLS,	icon_tools },
	{ PIXBUF_INLINE_ICON_VIEW,	icon_view }, { NULL, NULL }
};
...
void pixbuf_draw_triangle(GdkPixbuf *pb,
	...
	if (slope1) slope1 = (gdouble)(x2 - x1)
		/ slope1;
	slope1_x = x1;
	slope1_y = y1;
	slope2 = (gdouble)(y3 - y1);
	if (slope2) slope2 = (gdouble)(x3 - x1)
		/ slope2;
	...
	for (y = fy1; y < fy2; y++) {
		...
		if (!middle && y > y2) {
			...
			if (slope1) slope1 = (gdouble)(x3 - x2)
				/ slope1;
			slope1_x = x2;
			slope1_y = y2;
...
static gint util_clip_line(gdouble clip_x, gdouble clip_y, gdouble clip_w, gdouble clip_h,
	...
	if (x2 < clip_x || x1 > clip_x + clip_w)
		return FALSE;

	if (y1 < y2) {
		if (y2 < clip_y || y1 > clip_y + clip_h)
			return FALSE;
	}
	else {
		if (y1 < clip_y || y2 > clip_y + clip_h)
			return FALSE;
	}
...
static void pixbuf_draw_fade_linear(guchar *p_pix, gint prs, gint p_alpha,
	...
	for (j = y1; j < y2; j++) {
		...
		if (!vertical) n = a - a * abs(j - s)
			/ border;
		for (i = x1; i < x2; i++) {
			if (vertical) n = a - a * abs(i - s)
				/ border;
			*pp = (r * n + *pp * (256-n)) >> 8;
			pp++;
			*pp = (g * n + *pp * (256-n)) >> 8;
