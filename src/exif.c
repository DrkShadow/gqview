...
gchar *exif_text_list_find_value(ExifTextList *list, guint value) {
	...
	while (!result && list[i].value >= 0) {
		if (value == list[i].value)
			result = g_strdup(_(list[i].description));
		i++;
	}
	if (!result)
		result = g_strdup_printf("%d (%s)", value, _("unknown"));
	...
	switch (item->format) {
		...
		case EXIF_FORMAT_DOUBLE:
			for (i = 0; i < ne; i++) {
				...
				if (r.den)
					((double *)dest)[i] = (double)r.num / r.den;
			}
			break;
	}
}
...
gint exif_parse_IFD_table(ExifData *exif,
	...
	if (size < offset + count * EXIF_TIFD_SIZE + 4)
		return -1;
...
gint exif_tiff_parse(ExifData *exif, unsigned char *tiff, guint size, ExifMarker *list) {
	...
	if (!exif_tiff_directory_offset(tiff, size, &offset, &bo))
		return -1;
...
double exif_rational_to_double(ExifRational *r, gint sign) {
	...
	if (sign) return (double)((int)r->num)
		/ (double)((int)r->den);
...
static gchar *exif_get_formatted_by_key(ExifData *exif, const gchar *key, gint *key_valid) {
	...
	if (strcmp(key, "fDateTime") == 0) {
		...
		gchar *subsec = NULL;
		if (text)
			subsec = exif_get_data_as_text(exif, "SubsecTimeOriginal");
		if (!text) {
			text = exif_get_data_as_text(exif, "DateTime");
			if (text)
				subsec = exif_get_data_as_text(exif, "SubsecTime");
		}
		if (subsec) {
			gchar *tmp = text;
	...
	if (strcmp(key, "fAperture") == 0) {
		...
		if (n == 0.0)
			n = exif_get_rational_as_double(exif, "ApertureValue");
		if (n == 0.0)
			return NULL;
	...
	if (strcmp(key, "fISOSpeedRating") == 0) {
		...
		if (!text)
			text = exif_get_data_as_text(exif, "ExposureIndex");
		return text;
	}
	...
	if (strcmp(key, "fFlash") == 0) {
		...
		if (!exif_get_integer(exif, "Flash", &n))
			return NULL;

		/* Exif 2.1 only defines first 3 bits */
		if (n <= 0x07)
			return exif_text_list_find_value(ExifFlashList, n);
		...
		v = (n >> 3) & 0x03;
		if (v)
			string = append_comma_text(string, _("mode:"));
		switch (v) {
			case 1:
		...
		v = (n >> 1) & 0x03;
		if (v == 2)
			string = append_comma_text(string, _("not detected by strobe"));
		if (v == 3)
			string = append_comma_text(string, _("detected by strobe"));

		/* we ignore flash function (bit 5) */

		/* red-eye (bit 6) */
		if ((n >> 5) & 0x01)
			string = append_comma_text(string, _("red-eye reduction"));
...
const gchar *exif_get_description_by_key(const gchar *key) {
	...
	while (ExifKnownMarkersList[i].tag > 0) {
		if (strcmp(key, ExifKnownMarkersList[i].key) == 0)
			return _(ExifKnownMarkersList[i].description);
		i++;
	}
