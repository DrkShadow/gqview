...
static gint thumb_loader_mark_failure(ThumbLoader *tl) {
	...
	if (cache_ensure_dir_exists(cache_dir, mode)) {
		...
		if (debug)
			printf("marking thumb failure: %s\n", cache_path);
...
static void thumb_loader_done_cb(ImageLoader *il, gpointer data) {
	...
	if (tl->cache_hit && pw != tl->max_w && ph != tl->max_h) {
		...
		if (debug)
			printf("thumbnail size mismatch, regenerating: %s\n", tl->path);
		tl->cache_hit = FALSE;
...
static void thumb_loader_delay_done(ThumbLoader *tl) {
	if (tl->idle_done_id == -1)
		tl->idle_done_id = g_idle_add(thumb_loader_done_delay_cb, tl);
}
...
static void thumb_loader_setup(ThumbLoader *tl, gchar *path) {
	...
	if (tl->func_progress)
		image_loader_set_percent_func(tl->il, thumb_loader_percent_cb, tl);
}
		...
		if (cache_path) {
			if (cache_time_valid(cache_path, tl->path)) {
				...
				if (filesize(cache_path) == 0) {
					if (debug)
						printf("Broken image mark found:%s\n", cache_path);
					g_free(cache_path);
					return FALSE;
				}
...
void thumb_loader_free(ThumbLoader *tl) {
	...
	if (tl->idle_done_id != -1)
		g_source_remove(tl->idle_done_id);
