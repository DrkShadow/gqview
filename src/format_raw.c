...
static FormatRawEntry format_raw_list[] = {
	...
	FORMAT_RAW_NIKON,
	FORMAT_RAW_OLYMPUS,
	FORMAT_RAW_PENTAX, { NULL, 0, 0, NULL, 0, 0, NULL, NULL, NULL }
};
...
static FormatExifEntry format_exif_list[] = {
	...
	FORMAT_EXIF_FUJI,
	FORMAT_EXIF_NIKON,
	FORMAT_EXIF_OLYMPUS, { 0, NULL, 0, NULL }
};
...
static guint tiff_table(unsigned char *data, const guint len, guint offset, ExifByteOrder bo,
	...
	if (len < offset + 2)
		return 0;
	if (type < 0 || type > EXIF_FORMAT_COUNT)
		return 0;

	count = exif_byte_get_int16(data + offset, bo);
	offset += 2;
	if (len < offset + count * 12 + 4)
		return 0;
...
static gint format_raw_parse(FormatRawEntry *entry,
	...
	if (!entry || !entry->func_parse)
		return FALSE;

	if (debug)
		printf("RAW using file parser for %s\n", entry->description);
...
FormatRawExifType format_raw_exif_offset(unsigned char *data, const guint len, guint *exif_offset,
	...
	if (!format_raw_parse(entry, data, len, NULL, exif_offset))
		return FORMAT_RAW_EXIF_NONE;
...
gint format_raw_img_exif_offsets_fd(int fd, const gchar *path,
	...
	if (path) {
		...
		if (debug)
			printf("RAW file parser extension match\n");
	}
...
gint format_exif_makernote_parse(ExifData *exif, unsigned char *tiff, guint offset,
	...
	if (debug)
		printf("EXIF using makernote parser for %s\n", entry->description);
