...
static const gchar *print_output_name(PrintOutput output) {
	if (output < 0 || output >= PRINT_OUTPUT_COUNT)
		return "";
...
PaperSize print_paper_sizes[] = { { N_("Custom"),		360,	720,	PAPER_ORIENTATION_PORTRAIT },
	{ N_("Letter"),		612,	792,	PAPER_ORIENTATION_PORTRAIT },	/* in 8.5 x 11 */ { N_("Legal"),		612,	1008,	PAPER_ORIENTATION_PORTRAIT },	/* in 8.5 x 14 */
	{ N_("Executive"),	522,	756,	PAPER_ORIENTATION_PORTRAIT },	/* in 7.25x 10.5 */ { "A0",			2384,	3370,	PAPER_ORIENTATION_PORTRAIT },	/* mm 841 x 1189 */
	{ "A1",			1684,	2384,	PAPER_ORIENTATION_PORTRAIT },	/* mm 594 x 841 */ { "A2",			1191,	1684,	PAPER_ORIENTATION_PORTRAIT },	/* mm 420 x 594 */
	{ "A3",			842,	1191,	PAPER_ORIENTATION_PORTRAIT },	/* mm 297 x 420 */ { "A4",			595,	842,	PAPER_ORIENTATION_PORTRAIT },	/* mm 210 x 297 */
	{ "A5",			420,	595,	PAPER_ORIENTATION_PORTRAIT },	/* mm 148 x 210 */ { "A6",			298,	420,	PAPER_ORIENTATION_PORTRAIT },	/* mm 105 x 148 */
	{ "B3",			1001,	1417,	PAPER_ORIENTATION_PORTRAIT },	/* mm 353 x 500 */ { "B4",			709,	1001,	PAPER_ORIENTATION_PORTRAIT },	/* mm 250 x 353 */
	{ "B5",			499,	709,	PAPER_ORIENTATION_PORTRAIT },	/* mm 176 x 250 */ { "B6",			354,	499,	PAPER_ORIENTATION_PORTRAIT },	/* mm 125 x 176 */
	{ N_("Envelope #10"),	297,	684,	PAPER_ORIENTATION_LANDSCAPE },	/* in 4.125 x 9.5 */ { N_("Envelope #9"),	279,	639,	PAPER_ORIENTATION_LANDSCAPE },	/* in 3.875 x 8.875 */
	{ N_("Envelope C4"),	649,	918,	PAPER_ORIENTATION_LANDSCAPE },	/* mm 229 x 324 */ { N_("Envelope C5"),	459,	649,	PAPER_ORIENTATION_LANDSCAPE },	/* mm 162 x 229 */
	{ N_("Envelope C6"),	323,	459,	PAPER_ORIENTATION_LANDSCAPE },	/* mm 114 x 162 */ { N_("Photo 6x4"),	432,	288,	PAPER_ORIENTATION_PORTRAIT },	/* in 6   x 4 */
	{ N_("Photo 8x10"),	576,	720,	PAPER_ORIENTATION_PORTRAIT },	/* in 8   x 10 */ { N_("Postcard"),	284,	419,	PAPER_ORIENTATION_LANDSCAPE },	/* mm 100 x 148 */
	{ N_("Tabloid"),	792,	1224,	PAPER_ORIENTATION_PORTRAIT },	/* in 11  x 17 */ { NULL, 0, 0, 0 }
};
...
static void print_proof_size(PrintWindow *pw, gdouble *width, gdouble *height) {
	if (width)
		*width = pw->proof_width + PRINT_PROOF_MARGIN * 2;
	if (height) {
		gdouble h;

		h = pw->proof_height + PRINT_PROOF_MARGIN * 2;
		if (pw->text_fields != 0)
			h += PRINT_TEXT_PADDING;
		if (pw->text_fields & TEXT_INFO_FILENAME)
			h+= (gdouble)pw->text_points * 1.25;
...
static gint print_layout_page_count(PrintWindow *pw) {
	...
	if (images < 1 || images_per_page < 1)
		return 0;

	pages = images / images_per_page;
	if (pages * images_per_page < images)
		pages++;
...
static GList *print_window_list_printers(void) {
	...
	while (fgets(buffer, sizeof(buffer), p) != NULL) {
		...
		ptr = buffer;
		if (strncmp(ptr, "printer ", 8) != 0)
			continue;
		if (strstr(ptr, "enabled") == NULL)
			continue;
		ptr += 8;
		end = ptr;
		while (*end != '\0' && *end != '\n' && *end != ' ' && *end != '\t') end++;
...
static void pipe_handler_free(PipeError *pe) {
	if (!pe)
		return;
	if (pe != pipe_handler_data)
		printf("warning SIGPIPE handler not closing same data\n");
...
static gint print_job_ps_init(PrintWindow *pw) {
	...
	if (pw->job_file != NULL || pw->job_pipe != NULL)
		return FALSE;
	...
	if (!ret)
		print_job_throw_error(pw, _("SIGPIPE error writing to printer."));
...
static void print_job_throw_error(PrintWindow *pw, const gchar *message) {
	...
	if (GTK_WIDGET_VISIBLE(pw->dialog->dialog))
		parent = pw->dialog->dialog;
...
static void print_job_render_image_loader_done(ImageLoader *il, gpointer data) {
	...
	if (pixbuf) {
		...
		if (y + h >= pw->layout_height)
			h = pw->layout_height - y;
...
static void print_job_render_proof_loader_done(ImageLoader *il, gpointer data) {
	...
	if (pw->proof_point)
		pw->proof_point = pw->proof_point->next;
...
static GtkWidget *print_combo_menu(const gchar *text[], gint count, gint preferred,
	...
	if (func)
		g_signal_connect(G_OBJECT(combo), "changed", func, data);
...
static gint print_pref_int(const gchar *key, gint fallback) {
	...
	if (pref_list_int_get(PRINT_PREF_GROUP, key, &value))
		return value;
	return fallback;
}
...
static gdouble print_pref_double(const gchar *key, gdouble fallback) {
	...
	if (pref_list_double_get(PRINT_PREF_GROUP, key, &value))
		return value;
	return fallback;
}
