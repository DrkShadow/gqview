...
gint collection_info_load_thumb(CollectInfo *ci) {
	...
	return FALSE;
#if 0	
	if (create_thumbnail(ci->path, &ci->pixmap, &ci->mask) < 0)
		return FALSE;

	if (ci->pixmap)
		gdk_pixmap_ref(ci->pixmap);
	if (ci->mask)
		gdk_bitmap_ref(ci->mask);
...
CollectWindow *collection_window_find_by_path(const gchar *path) {
	...
	while (work) {
		CollectWindow *cw = work->data;
		if (cw->cd->path && strcmp(cw->cd->path, path) == 0)
			return cw;
		work = work->next;
	}
...
void collection_ref(CollectionData *cd) {
	...
	if (debug)
		printf("collection \"%s\" ref count = %d\n", cd->name, cd->ref);
}
...
CollectionData *collection_from_dnd_data(const gchar *data, GList **list, GList **info_list) {
	...
	if (strncmp(data, "COLLECTION:", 11) != 0)
		return NULL;
	...
	if (!cd || (!list && !info_list)) {
		...
		while (data[b] != '\0') {
			...
			if (info && list)
				work = g_list_append(work, g_strdup(info->path));
			if (info && info_list)
				infol = g_list_append(infol, info);
...
static void collection_remove_by_info(CollectionData *cd, CollectInfo *info) {
	if (!info || !g_list_find(cd->list, info))
		return;
...
CollectWindow *collection_window_new(const gchar *path) {
	...
	if (path && *path == '/')
		collection_load_begin(cw->cd, NULL, FALSE);
