...
static void config_window_apply(void) {
	...
	if (buf && strlen(buf) > 0)
		startup_path = remove_trailing_slash(buf);

	g_free(safe_delete_path);
	safe_delete_path = NULL;
	buf = gtk_entry_get_text(GTK_ENTRY(safe_delete_path_entry));
	if (buf && strlen(buf) > 0)
		safe_delete_path = remove_trailing_slash(buf);

	if (show_dot_files != show_dot_files_c)
		refresh = TRUE;
	if (file_sort_case_sensitive != file_sort_case_sensitive_c)
		refresh = TRUE;
	if (file_filter_disable != file_filter_disable_c)
		refresh = TRUE;
	...
	for (i = 0; i < COLOR_PROFILE_INPUTS; i++) {
		...
		if (buf && strlen(buf) > 0)
			color_profile_input_name[i] = g_strdup(buf);
		...
		if (buf && strlen(buf) > 0)
			color_profile_input_file[i] = g_strdup(buf);
	}
	...
	if (buf && strlen(buf) > 0)
		color_profile_screen_file = g_strdup(buf);
#endif
...
static void safe_delete_clear_cb(GtkWidget* widget, gpointer data) {
	...
	if (safe_delete_path)
		gtk_entry_set_text(GTK_ENTRY(entry), safe_delete_path);
...
static void config_window_create(void) {
	...
	for (i = 0; i < GQVIEW_EDITOR_SLOTS; i++) {
		...
		if (editor_name[i])
			gtk_entry_set_text(GTK_ENTRY(editor_name_entry[i]),editor_name[i]);
		...
		if (editor_command[i])
			gtk_entry_set_text(GTK_ENTRY(editor_command_entry[i]), editor_command[i]);
	...
	for (i = 0; i < COLOR_PROFILE_INPUTS; i++) {
		...
		if (color_profile_input_name[i])
			gtk_entry_set_text(GTK_ENTRY(entry), color_profile_input_name[i]);
