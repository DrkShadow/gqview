...
static void dupe_window_update_progress(DupeWindow *dw, const gchar *status, gdouble value, gint force) {
	...
	if (status) {
		...
		if (!force &&
			...
			if (new_time - dw->setup_time_count < 250000)
				return;
			...
			if (dw->setup_time_count == 0)
				dw->setup_time_count = 1;
			if (new_time > 0)
				dw->setup_time_count = new_time;
			gtk_progress_bar_set_fraction(GTK_PROGRESS_BAR(dw->extra_label), value);
			status_text = status;
...
static DupeItem *dupe_item_find_path(DupeWindow *dw, const gchar *path) {
	...
	if (!di && dw->second_set)
		di = dupe_item_find_path_by_list(path, dw->second_list);
...
static void dupe_item_write_cache(DupeItem *di) {
	...
	if (cache_ensure_dir_exists(base, mode)) {
		...
		if (di->checksum != 0)
			cache_sim_data_set_checksum(cd, di->checksum);
		if (di->md5sum) {
			guchar digest[16];
...
static DupeItem *dupe_match_find_parent(DupeWindow *dw, DupeItem *child) {
	...
	if (g_list_find(dw->dupes, child))
		return child;

	work = child->group;
	while (work) {
		DupeMatch *dm = work->data;
		if (g_list_find(dw->dupes, dm->di))
			return dm->di;
		work = work->next;
	}
...
static void dupe_match_reparent(DupeWindow *dw, DupeItem *old, DupeItem *new) {
	...
	if (!old || !new || !dupe_match_link_exists(old, new))
		return;
...
static GList *dupe_match_group_trim(GList *list, DupeWindow *dw) {
	...
	while (work) {
		DupeItem *di = work->data;
		if (!di->second)
			list = dupe_match_group_filter(list, di, dw);
		work = work->next;
		if (di->second)
			list = g_list_remove(list, di);
	}
...
static gint dupe_match_rank_sort_cb(gconstpointer a, gconstpointer b) {
	...
	if (da->group_rank > db->group_rank)
		return -1;
	if (da->group_rank < db->group_rank)
		return 1;
	return 0;
...
static void dupe_match_rank(DupeWindow *dw) {
	...
	if (debug > 1)
		dupe_match_print_list(list);

	if (debug)
		printf("Similar items: %d\n", g_list_length(list));
	list = dupe_match_group_trim(list, dw);
	if (debug)
		printf("Unique groups: %d\n", g_list_length(list));
...
static gint dupe_match(DupeItem *a, DupeItem *b, DupeMatchType mask, gdouble *rank, gint fast) {
	*rank = 0.0;
	...
	if (mask & DUPE_MATCH_SUM) {
		if (!a->md5sum)
			a->md5sum = md5_text_from_file_utf8(a->path, "");
		if (!b->md5sum)
			b->md5sum = md5_text_from_file_utf8(b->path, "");
		if (a->md5sum[0] == '\0' ||
		    b->md5sum[0] == '\0' ||
	...
	if (mask & DUPE_MATCH_DIM) {
		if (a->width == 0)
			image_load_dimensions(a->path, &a->width, &a->height);
		if (b->width == 0)
			image_load_dimensions(b->path, &b->width, &b->height);
...
static void dupe_listview_set_thumb(DupeWindow *dw, DupeItem *di, GtkTreeIter *iter) {
	...
	if (iter)
		gtk_list_store_set(store, iter, DUPE_COLUMN_THUMB, di->pixbuf, -1);
}

static void dupe_thumb_do(DupeWindow *dw) {
	DupeItem *di;

	if (!dw->thumb_loader || !dw->thumb_item)
		return;
	di = dw->thumb_item;
...
static void dupe_thumb_step(DupeWindow *dw) {
	...
	while (!di && valid) {
		...
		if (pixbuf || di->pixbuf) {
			if (!pixbuf)
				gtk_list_store_set(GTK_LIST_STORE(store), &iter, DUPE_COLUMN_THUMB, di->pixbuf, -1);
			row++;
			di = NULL;
		}
		valid = gtk_tree_model_iter_next(store, &iter);
	...
	if (!thumb_loader_start(dw->thumb_loader, di->path)) {
		...
		if (debug)
			printf("error loading thumb for %s\n", di->path);
		dupe_thumb_do(dw);
		dupe_thumb_step(dw);
	}
}
...
static GList *dupe_setup_point_step(DupeWindow *dw, GList *p) {
	...
	if (dw->second_set && g_list_first(p) == dw->list)
		return dw->second_list;
...
static void dupe_check_start(DupeWindow *dw) {
	...
	dw->setup_count = g_list_length(dw->list);
	if (dw->second_set)
		dw->setup_count += g_list_length(dw->second_list);
...
static void dupe_menu_view_cb(GtkWidget *widget, gpointer data) {
	...
	if (dw->click_item)
		dupe_menu_view(dw, dw->click_item, dw->listview, FALSE);
}
...
static void dupe_menu_viewnew_cb(GtkWidget *widget, gpointer data) {
	...
	if (dw->click_item)
		dupe_menu_view(dw, dw->click_item, dw->listview, TRUE);
}
...
static void dupe_second_menu_view_cb(GtkWidget *widget, gpointer data) {
	...
	if (dw->click_item)
		dupe_menu_view(dw, dw->click_item, dw->second_listview, FALSE);
}
...
static void dupe_second_menu_viewnew_cb(GtkWidget *widget, gpointer data) {
	...
	if (dw->click_item)
		dupe_menu_view(dw, dw->click_item, dw->second_listview, TRUE);
}
...
static GtkTargetEntry dupe_drag_types[] = { { "text/uri-list", 0, TARGET_URI_LIST },
	{ "text/plain", 0, TARGET_TEXT_PLAIN }
};
static gint n_dupe_drag_types = 2;

static GtkTargetEntry dupe_drop_types[] = { { "application/x-gqview-collection-member", 0, TARGET_APP_COLLECTION_MEMBER },
...
static void dupe_dnd_data_get(GtkWidget *widget, GdkDragContext *context,
	...
	if (source == dw->listview || source == dw->second_listview)
		return;
